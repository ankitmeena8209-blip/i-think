...
async function runTests() {
  ...
  try {
    const tables = (await db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all()).map(t => t.name);
  ...
  try {
    const adminUsername = process.env.ADMIN_USER || 'im_ankiit';
    const adminPassword = process.env.ADMIN_PASS || '82090760107200ankitbeingfrzi';
    ...
    const adminUser = await db.prepare('SELECT * FROM users WHERE username = ?').get(adminUsername);
  ...
  try {
    ...
    await db.prepare('DELETE FROM users WHERE username = ?').run(testUsername);
    const uRes = await db.prepare('INSERT INTO users (username, word1, word2, is_admin) VALUES (?, \'Admin\', \'TestUser\', 0)').run(testUsername);
    const testUserId = uRes.lastInsertRowid;

    const tRes = await db.prepare('INSERT INTO thoughts (user_id, username, content) VALUES (?, ?, \'Test thought for admin delete\')').run(testUserId, testUsername);
    const testThoughtId = tRes.lastInsertRowid;

    const userResults = await db.prepare('SELECT u.id, u.username, COUNT(t.id) as thought_count FROM users u LEFT JOIN thoughts t ON u.id = t.user_id WHERE u.username LIKE ? GROUP BY u.id').all(`%${testUsername}%`);
    assert(userResults.length > 0 && userResults[0].thought_count >= 1, 'Users query computes thought count');

    // Delete all user thoughts
    await db.prepare('DELETE FROM thoughts WHERE user_id = ?').run(testUserId);
    const checkThoughts = await db.prepare('SELECT * FROM thoughts WHERE user_id = ?').all(testUserId);
    ...
    const checkUser = await db.prepare('SELECT * FROM users WHERE id = ?').get(testUserId);
  ...
  try {
    const uRes = await db.prepare('INSERT INTO users (username, word1, word2, is_admin) VALUES (\'BulkUser\', \'Bulk\', \'User\', 0)').run();
    const uid = uRes.lastInsertRowid;

    const t1 = (await db.prepare('INSERT INTO thoughts (user_id, username, content) VALUES (?, \'BulkUser\', \'T1\')').run(uid)).lastInsertRowid;
    const t2 = (await db.prepare('INSERT INTO thoughts (user_id, username, content) VALUES (?, \'BulkUser\', \'T2\')').run(uid)).lastInsertRowid;

    await db.prepare('DELETE FROM thoughts WHERE id IN (?, ?)').run(t1, t2);
    const checkBulk = await db.prepare('SELECT * FROM thoughts WHERE id IN (?, ?)').all(t1, t2);
  ...
  try {
    const mRes = await db.prepare('INSERT INTO contact_messages (username, message, status) VALUES (\'MsgUser\', \'Test msg\', \'pending_retry\')').run();
    const mid = mRes.lastInsertRowid;

    await db.prepare('UPDATE contact_messages SET status = \'resolved\' WHERE id = ?').run(mid);
    const resolvedMsg = await db.prepare('SELECT status FROM contact_messages WHERE id = ?').get(mid);
    assert(resolvedMsg.status === 'resolved', 'Mark Message as Resolved updates message status');

    await db.prepare('DELETE FROM contact_messages WHERE id = ?').run(mid);
  } catch (err) {
